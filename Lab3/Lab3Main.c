...
double triangle(double edge1, double edge2, double edge3) // —уществование треугольника.
{
	int exist = 0;
...
double angle(double edge1, double edge2, double edge3) // ”гол между edge2 и edge3.
{

	double cosAng = (edge2 * edge2 + edge3 * edge3 - edge1 * edge1) / (2 * edge2 * edge3);
	return acos(cosAng) * 180 / PI;
}
void printAngle(double angle) // ѕеревод дробной части угла в секунды и минуты.
{
